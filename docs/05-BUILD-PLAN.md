...
- **`android/` and `ios/` are generated by `expo prebuild` and never hand edited.** Every
  native change is a config plugin **or a local Expo module in `modules/`**, both of which
  survive `prebuild --clean`. A hand patched manifest is silently lost on the next prebuild.
  **A config plugin can only DECLARE, never implement** — Slice 6 spent a day believing it
  had a foreground service because a plugin had declared a `<service>` for a class that did
  not exist. Anything that needs real native code needs a module
...
- **Filed against Slice 11, found 2026-09-19:** `MainActivity`'s `configChanges` does not
  list `fontScale` or `density`, so changing either **restarts the app**. A reader who adjusts
  their text size while a timer runs loses the running timer and meets the recovery sheet.
  App-wide and true since Slice 0; the font pass is where React Native should be made to
  handle the change instead of restarting. `DECISIONS.md`, 2026-09-19.
...
- Foreground service so it survives backgrounding, via a **local Expo module** — a config
  plugin cannot supply a service class, only declare one
...
**Status, 2026-09-19: on a real phone, and the central claim of the slice was false.**
The Nothing Phone 2a run found that **there was no foreground service at all**:
`plugins/withReadingService.js` declared a `<service>` naming a class that does not exist in
`expo-notifications` and appears 0 times in the built APK. `dumpsys activity services` said
`(nothing)`; the backgrounded process was `cch` at `oom_score_adj` 900, first to be killed.
Everything else in the slice was resting on it.

Fixed with `modules/reading-service`, a local Expo module wrapping a real Kotlin `Service`.
Re-measured on the phone, backgrounded: `fg +50 F/S/FGS (fg-service-act)`, adj **50**. Four
smaller bugs came out of the same session — no notification on a reader's first ever session,
the `AppState` listener still living in the screen, "Not now" not remembered, and a Finish
button wired to nothing. `docs/device-checks/slice-6.md` has the numbers.

**Status, 2026-09-18: built, emulator only, and NOT done.** Two-week clock started 2026-09-18.
- **Verified on Pixel_7_API_36:** the ring and clock, Start writing the session row first,
  resume-not-restart, Pause/Resume, Finish saving a duration that matches the screen, and the
  crash path raising Slice 1's recovery sheet. **The decisive one:** 53 seconds read, app shut
  4 minutes, sheet offered 1 minute rather than 5 — the heartbeat bound reaches the reader.
- **The foreground service, the notification with Pause and Finish, and permission priming
  are all BUILT.** The blocking peer conflict was drift, not an incompatibility: the tree held
  react-dom one minor ahead of what SDK 57 pins, corrected with an `overrides` entry.
- **Three silent notification bugs were found and fixed on the emulator**: never presented
  (no handler), on the wrong channel (importance 4 fallback), and re-posted after Finish.
  None threw; all three were found by reading `dumpsys` and the shade.
- **The done-when above is HALF met:** force-stop and reopen does offer the elapsed time, on
  an emulator. Neither Xiaomi nor Samsung has seen it, and that is the half that matters.
- Details: `DECISIONS.md` 2026-09-18, and `docs/device-checks/slice-6.md`.
...
1. Barcode scanning, the top deferred request. **Moved here deliberately on 2026-09-18**,
   together with photo covers and "Snap a page" in notes: all three need the camera, one
   native rebuild and one permission. **Three decisions did NOT move with them** — see
   `DECISIONS.md`, 2026-09-18: what Slice 8 does with file-shaped data on another device
   (already wrong today for `cover_local_path`), whether photo covers are free or Plus
   (Slice 10 writes the paywall), and that the permission then lands closest to release
